...
exports.findGitHubUrl = findGitHubUrl;
...
async function findGitHubUrl(name, company, existingUrl) {
    if (existingUrl)
        return existingUrl;
    try {
        ...
        return all.map((r) => r.url || r.link || "").find((url) => url.includes("github.com/") && !url.includes("github.com/search")) || null;
    }
    catch (err) {
...
async function quickGitHubScan(username) {
    try {
        const res = await fetch(`${NIA_BASE}/v2/search/web`, {
            method: "POST",
            headers: niaHeaders(),
        ...
        return results.slice(0, 3)
            .map((r) => r.snippet || r.description || r.title || "")
            .filter(Boolean)
            .join(" | ");
    }
    catch {
...
async function deepTracerWithTimeout(username, githubUrl, jobDescription, timeoutMs = 25000) {
    try {
            ...
            try {
                ...
                for (const line of fullText.split("\n")) {
                    ...
                    try {
                        ...
                        if (content && content.length > bestResult.length)
                            bestResult = content;
                    }
                    catch { }
                }
...
async function traceGitHub(githubUrl, jobDescription) {
